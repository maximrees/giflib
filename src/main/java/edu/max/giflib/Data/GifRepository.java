...
private static final List<Gif> ALL_GIFS = Arrays.asList(
        ...
        public List<Gif> getAllGifsOfCategory(int id){
            List<Gif> categoryGifs = new ArrayList<Gif>();
            for( Gif gif : ALL_GIFS){
                if(gif.getCategoryID() == id) categoryGifs.add(gif);
            }
            return categoryGifs;
        }
        ...
        public List<Gif> getAllFavorites(){
            return getAllGifs().stream().filter(Gif::isFavorite).collect(Collectors.toList());
        }
}
